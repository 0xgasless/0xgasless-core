...
import { NotConnectedError, SigningRejectedError, SigningTimeoutError, WidgetError, } from "./errors";
import { DEFAULT_WIDGET_URL, DEFAULT_LOGIN_TIMEOUT, DEFAULT_SIGN_TIMEOUT, DEFAULT_Z_INDEX, IFRAME_ID, IFRAME_READY_DELAY, NETWORK_NAMES, } from "./constants";
 ...
 * ```
 */
export class OxGasAuth extends TypedEventEmitter {
    ...
    async login() {
        ...
        const loginPromise = new Promise((resolve, reject) => {
            const handleMessage = (event) => {
                ...
                if (type === "WIDGET_READY") {
                    ...
                    const error = new WidgetError(payload?.message || "Authentication failed.");
        ...
        return withTimeout(loginPromise, this.loginTimeout, () => new SigningTimeoutError(this.loginTimeout, "Login"));
    }
    /**
    ...
    async signTransaction(txParams) {
        ...
        const signPromise = new Promise((resolve, reject) => {
            const handleMessage = (event) => {
                ...
                if (type === "TX_SIGNED") {
                    ...
                    window.removeEventListener("message", handleMessage);
                    this.hideIframe();
                    const error = new WidgetError(payload?.message || "Transaction signing failed.");
                    this.emit("error", error);
                    reject(error);
                }
            };
        ...
        return withTimeout(signPromise, this.signTimeout, () => new SigningTimeoutError(this.signTimeout, "Transaction signing"));
    }
    /**
    ...
    async signMessage(message, network) {
        ...
        const signPromise = new Promise((resolve, reject) => {
            const handleMessage = (event) => {
                ...
                if (type === "MSG_SIGNED") {
                    ...
                    window.removeEventListener("message", handleMessage);
                    this.hideIframe();
                    const error = new WidgetError(payload?.message || "Message signing failed.");
                    this.emit("error", error);
                    reject(error);
                }
            };
        ...
        return withTimeout(signPromise, this.signTimeout, () => new SigningTimeoutError(this.signTimeout, "Message signing"));
    }
    /**
