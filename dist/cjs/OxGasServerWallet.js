...
exports.OxGasServerWallet = void 0;
const server_wallet_errors_1 = require("./server-wallet-errors");
// ─── Defaults ────────────────────────────────────────────────────
...
class OxGasServerWallet {
    ...
    async broadcastTransaction(rawTx, rpcUrl) {
        ...
        try {
            ...
            throw new server_wallet_errors_1.NetworkError(`Failed to broadcast transaction: ${err.message}`);
        }
        if (!response.ok) {
            throw new server_wallet_errors_1.NetworkError(`RPC endpoint returned HTTP ${response.status}`, response.status);
        }
        const result = (await response.json());
        if (result.error) {
            throw new server_wallet_errors_1.NetworkError(`RPC error: ${result.error.message}`, result.error.code);
        }
    ...
    async request(method, path, body) {
        ...
        try {
            ...
            if (err.name === "TimeoutError" || err.name === "AbortError") {
                throw new server_wallet_errors_1.NetworkError(`Request to ${path} timed out after ${this.timeout}ms`);
            }
            throw new server_wallet_errors_1.NetworkError(`Failed to reach 0xgas API: ${err.message}`);
        }
        // ── Map HTTP error codes to typed errors ──
        if (response.status === 403) {
            const data = await this.safeJson(response);
            const reason = data?.error ?? data?.message ?? "Unknown policy violation";
            throw new server_wallet_errors_1.PolicyViolationError(reason);
        }
        if (response.status === 404) {
            ...
            if (uid) {
                throw new server_wallet_errors_1.WalletNotFoundError(uid);
            }
            throw new server_wallet_errors_1.NetworkError(data?.error ?? data?.message ?? `Not found: ${path}`, 404);
        }
        if (!response.ok) {
            const data = await this.safeJson(response);
            ...
            throw new server_wallet_errors_1.ServerSigningError(msg);
        }
...
exports.OxGasServerWallet = OxGasServerWallet;
